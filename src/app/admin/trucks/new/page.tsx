...
import { addTruck } from "@/services/post";
...
export default function Page() {
  ...
  const onSubmit: SubmitHandler<Inputs> = async ({ image, name, price }) => {
    if (!imageUrl.error) {
      await mutateAsync({
  ...
  return (
    <main>
      ...
      <section className="container mt-10 px-4 mb-10">
        <h1 className="text-2xl sm:text-3xl font-bold">Add a new truck</h1>
        <p className="text-neutral-700 mb-5">
          Trucks added here will reflect in the truck list on the public site
        </p>
        <form
